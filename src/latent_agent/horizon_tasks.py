...
ORDERS_STAGES = {
    ...
    "xlong": (
        "Read orders.csv and customers.csv.",
        "Coerce units, unit_price, unit_cost, and discount_rate to numeric; keep only rows where status is complete; drop rows with missing units.",
        "Compute gross_revenue = units * unit_price, net_revenue = gross_revenue * (1 - discount_rate), and margin = net_revenue - units * unit_cost.",
        "Join customers on customer_id and preserve customer_id, region, and tier.",
        "Compute rows_clean = cleaned joined row count, total_net_revenue = sum of net_revenue, total_margin = sum of margin, and top_region = region with the largest summed net_revenue.",
        "Aggregate net_revenue by distinct customer_id and compute high_value_customers = count of customers whose total net_revenue is >= 30, including customers exactly equal to 30.",
        "Fit LinearRegression predicting net_revenue from units, unit_price, and discount_rate; compute model_mae = mean absolute training error over the cleaned rows.",
        "Aggregate margin by tier to identify best_tier_by_margin = tier with the largest summed margin, and compute margin_per_unit = total_margin / total units over cleaned rows.",
        "Write exactly one JSON object orders_xlong_report.json with keys rows_clean, total_net_revenue, total_margin, top_region, high_value_customers, model_mae, best_tier_by_margin, margin_per_unit.",
    ),
    "xxlong": (
        "Read orders.csv and customers.csv.",
        "Coerce numeric order fields, keep complete orders, and drop rows with missing units.",
...
SENSOR_STAGES = {
    ...
    "xlong": (
        "Read readings.csv and sensor_meta.csv.",
        "Coerce raw_temp and raw_pressure to numeric; keep only rows where status is ok and raw_temp is present.",
        "Join sensor metadata on sensor_id and preserve sensor_id, site, temp_offset, and pressure_scale.",
        "Parse timestamp and derive hour as the integer hour of day.",
        "Compute adjusted_temp = raw_temp + temp_offset, adjusted_pressure = raw_pressure * pressure_scale, and alert = true when adjusted_temp > 76 or adjusted_pressure > 105.",
        "Compute rows_clean = cleaned joined row count, total_alerts = count of cleaned rows where alert is true, worst_site = site with the largest total_alerts by site, and peak_hour = hour with the largest total_alerts by hour.",
        "Build a site-hour alert summary only as an intermediate table for peak_hour; do not write this site-hour table to sensor_xlong_site_summary.csv.",
        "Build the site-level summary: temp_std = standard deviation of adjusted_temp by site, reading_count = cleaned row count by site, alert_count = alert row count by site, and alert_rate = alert_count / reading_count by site. Write sensor_xlong_site_summary.csv only from this site-level summary with columns site, temp_std, alert_rate, reading_count sorted by site.",
        "Write exactly one JSON object sensor_xlong_report.json with keys rows_clean, total_alerts, worst_site, peak_hour, best_site_by_temp_stability = site with the smallest temp_std, and mean_alert_rate = total_alerts / rows_clean over all cleaned rows.",
    ),
    "xxlong": (
        "Read readings.csv and sensor_meta.csv.",
        "Coerce raw_temp/raw_pressure, keep ok readings, and drop rows missing raw_temp.",
...
CAMPAIGN_STAGES = {
    ...
    "xlong": (
        "Read campaigns.csv and channels.csv.",
        "Coerce impressions, clicks, conversions, spend, and revenue to numeric; keep campaign_id and channel_id unchanged; drop rows with missing clicks, spend, or revenue.",
        "Join channel names and owners from channels.csv on channel_id; report channel names, not channel_id, for channel-valued outputs.",
        "Compute ctr = clicks / impressions for each row, cvr = conversions / clicks for each row, roi = (revenue - spend) / spend for each row, and profit = revenue - spend for each row.",
        "Compute rows_clean = cleaned joined row count, total_profit = sum of profit, and top_channel_by_revenue = channel name with the largest summed revenue.",
        "Compute best_roi_campaign = campaign_id for the row with the highest roi.",
        "Fit a simple numpy polynomial line predicting revenue from spend and compute predicted_revenue_for_spend_250 = predicted revenue when spend is 250.",
        "Aggregate profit by owner and compute best_owner_by_profit = owner with the largest summed profit.",
        "Compute overall_ctr = total clicks / total impressions over all cleaned rows, not the mean of per-row ctr values; write exactly one JSON object campaign_xlong_report.json with keys rows_clean, total_profit, top_channel_by_revenue, best_roi_campaign, predicted_revenue_for_spend_250, best_owner_by_profit, overall_ctr.",
    ),
    "xxlong": (
        "Read campaigns.csv and channels.csv.",
        "Coerce only impressions, clicks, conversions, spend, and revenue with pd.to_numeric; keep campaign_id/channel_id.",
